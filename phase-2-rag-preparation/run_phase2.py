"""
Phase 2 pipeline: RAG preprocessing and semantic embedding index build.
User question -> Embedding model -> Vector similarity search -> Top chunks -> LLM.

Run from project root: python -m phase-2-rag-preparation.run_phase2
"""
...
try:  # pragma: no cover - import flexibility
    from .build_semantic_embeddings import build_and_save_semantic_embeddings
except ImportError:
    from build_semantic_embeddings import build_and_save_semantic_embeddings
...
def main() -> None:
    ...
    print(f"Phase 2 complete. Semantic index at {phase2_dir}")
